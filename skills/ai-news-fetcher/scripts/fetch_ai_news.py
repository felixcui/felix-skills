...
【重要原则】
1. 所有资讯都与AI/科技相关，请优先归入前9个分类，尽量减少"其他"分类的使用
...
**行业相关** - AI行业商业与战略动态
- 包含：公司动态、融资并购、产品战略、市场竞争
- 包含：人物访谈、深度分析、行业观察、趋势预测
- 包含：人事变动、财报、IPO、政策法规
...
def classify_by_keywords(news_list):
    ...
    for i, news in enumerate(news_list):
        ...
        if not matched:
            ...
            elif any(x in title for x in ["观点", "趋势", "思考", "预测", "深度", "访谈"]):
                categories["行业相关"].append(i)
            else:
                categories["行业相关"].append(i)
            classified.add(i)
