...
**CLI 来源**: [teng-lin/notebooklm-py](https://github.com/teng-lin/notebooklm-py)（⭐ 16.8k）— 非官方 Google NotebookLM Python API + CLI。通过 `pip install notebooklm-py` 安装。当前版本 0.3.4，安装在 Homebrew Python 3.14 下。

```python
NOTEBOOKLM = "/opt/homebrew/bin/python3.14 -m notebooklm"
NOTEBOOK_ID = "b08626a7-cda5-4dd2-b0e7-536eafb48274"  # "AI 资讯 V3"
NOTEBOOK_NAME = "AI 资讯 V3"
```

始终使用 NOTEBOOK_ID 而非 NOTEBOOK_NAME。名称匹配经常返回 `No notebook found`，且 `notebooklm list` 输出的 ID 可能被截断导致 `account-routing mismatch` 错误。硬编码完整 ID 是最可靠的方式。

**迁移历史**：87c6e099(V1, 273满) → 8c8a9ffe(V2, 300满) → b08626a7(V3, 当前活跃)。当出现 `Failed to get SOURCE_ID from registration response` 时，说明 source 数量已达上限，需新建笔记本并更新此处 ID。
...
| `category` | 自动分类：AI算力/模型技术/Agent基建/AI软件工程/内容创作/个人生产力/行业应用/智能终端/行业相关/其他 |
| updatetime | 当前时间 |
...
- 抓取阶段"成功"但原始正文极短（仅 UI 文本片段如"登录""生成动画"等）
- `"summary": "。"` 或 `"summary": ""`

**处理流程**：
1. 用 `browser_navigate` 打开 URL，`browser_snapshot(full=true)` 获取页面信息和功能描述
2. 结合页面标题、标语、功能列表、UI 文本等手动撰写产品介绍式摘要
3. 用 `patch` 工具更新本地 Markdown 文件（`~/work/github/media-conent/raw/` 下对应文件）的摘要和正文
4. 用 `lark-cli base +record-batch-create` 补推带正确摘要的记录到飞书多维表格
5. **告知用户**：自动流程已推送一条空摘要记录，表格中会有两条重复记录，由用户决定是否清理空摘要那条

**预防**（未来可加到 `collect_v2.py`）：检测原始正文 < 100 字符且摘要无效时，标记为非文章页面并提示 agent 手动处理，而非静默推送空摘要。
...
**处理流程**：
1. 用 `browser_navigate` 打开 URL，`browser_snapshot(full=true)` 获取页面信息和功能描述
2. 注意 SPA 页面可能有模态弹窗遮挡交互，需先通过 `browser_console` 隐藏或关闭
3. 结合页面标题、标语、功能列表、UI 文本等手动撰写产品介绍式摘要
4. 用 `patch` 工具更新本地 Markdown 文件（`~/work/github/media-conent/raw/` 下对应文件）的摘要和正文
5. 用 `lark-cli base +record-batch-create` 补推带正确摘要的记录到飞书多维表格
6. **告知用户**：自动流程已推送一条空/无效摘要记录，表格中会有两条重复记录，由用户决定是否清理
...
/opt/homebrew/bin/python3.14 -m notebooklm source add /tmp/ascii-name.md --notebook "b08626a7-cda5-4dd2-b0e7-536eafb48274"
...
**解决**：使用完整 notebook ID 而非名称。ID 为 `b08626a7-cda5-4dd2-b0e7-536eafb48274`（当前 V3）。
...
# ✅ 可靠 — 始终使用完整 ID
notebooklm source list --notebook "b08626a7-cda5-4dd2-b0e7-536eafb48274"
```
...
/opt/homebrew/bin/python3.14 -m notebooklm source add /tmp/english-name.md --notebook "b08626a7-cda5-4dd2-b0e7-536eafb48274"
```
...
**原因5：笔记本 source 数量达到上限（`Failed to get SOURCE_ID from registration response`）**
⚠️ **高复发率**：此问题已发生三次（V1@273, V2@300, 预计 V3 也终将满）。是 NotebookLM 的硬限制，非 bug。
当笔记本内 source 数量接近/超过上限（实测 273(V1)/300(V2) 个时触发），所有 `source add` 操作会稳定失败，返回 `Failed to get SOURCE_ID from registration response` 或 `RPC ADD_SOURCE failed`。`doctor` 检查全部通过，认证正常。
...
**处理**：
1. 新建笔记本（`notebooklm create "AI 资讯 V<n>"`），记录新 UUID
2. 更新三处配置：
   - `collect_v2.py` 中的 `NOTEBOOK_ID` 和 `NOTEBOOK_NAME`
   - `news-collect` SKILL.md 中的 `NOTEBOOK_ID` 和迁移历史
   - `llm-wiki references/daily-maintenance-cron.md` 中的 notebook ID
3. 可选：清理旧笔记本中的垃圾 source（`notebooklm_upload_*`、`upload_*`、`tmp*`、`nb-upload-*` 等残留）
4. 验证：用新 ID 测试上传一篇，确认成功
...
- 🔧 改进：思考泄漏检测覆盖更广 — 支持 `步骤/方法/要点/结构/背景/核心/关键/主要/特点/目标/思路/方向` 等更多分析关键词，以及多行占比>50%的检测
- 🔧 改进：摘要 prompt 优化 — 简化指令结构，用"禁止输出"替代"不要输出"，降低 LLM 跑偏概率
